...
class myApp(QtWidgets.QMainWindow):
    ...
    def send(self):
        ...
        kisi = self.dialog.comboBox.currentText()
        mesaj = self.dialog.lineEdit.text()


        sorgu = f"INSERT INTO `chat` (`k_gonderen`, `k_alici`, `mesaj`, `tarih`) VALUES ('{self.k_adi}', '{kisi}', '{mesaj}', CURRENT_TIMESTAMP); "
    ...
    def showmessage(self):
        self.dialog.textBrowser.clear()
        cursor = self.connection.cursor()
        kisi = self.dialog.comboBox.currentText()


        sorgu = f"Select tarih, mesaj from chat where k_gonderen = '{self.k_adi}' and k_alici='{kisi}' or k_gonderen = '{kisi}' and k_alici='{self.k_adi}' ORDER BY `chat`.`tarih` ASC"
        cursor.execute(sorgu)

        sonuclar = cursor.fetchall()

        for i in sonuclar:
            yazı = str(i[0])+"\t"+str(i[1])
            self.dialog.textBrowser.append(yazı)
    ...
    def girisYap(self):
        ...
        self.dialog.send.clicked.connect(self.send)
        self.dialog.comboBox.currentIndexChanged.connect(self.showmessage)
        self.showmessage()
